...
```jsx
// ❌ À éviter
function usercard({ user }) { // ❌ Doit commencer par une majuscule
  return <div>{user.name}</div>;
}

const productlist = ({ products }) => ( // ❌ Doit commencer par une majuscule
  <ul>...</ul>
);
```
...
function ProductList({ products }) {
  return (
    <ul>
      {products.map(product => (
        <li key={product.id}> {/* ✅ Clé unique et stable */}
  ...
  return (
    <ul>
      {items.map((item, index) => (
        <li key={index}> {/* ✅ Acceptable si pas d'ID unique */}
          {item}
        </li>
      ))}
    </ul>
...
function ProductList({ products }) {
  return (
    <ul>
      {products.map(product => (
        <li key={Math.random()}> {/* ❌ Clé changeante à chaque rendu */}
          {product.name}
        </li>
      ))}
    </ul>
...
function UserList({ users }) {
  return (
    <ul>
      {users.map(user => (
        <li key="user-item"> {/* ❌ Clé dupliquée */}
          {user.name}
        </li>
      ))}
    </ul>
...
// ❌ À éviter
// Pas de typage
const UserProfile = ({ user, onEdit }) => { // ❌ Types implicites any
  return (
    <div>
      <h2>{user.name}</h2>
...
test('renders without crashing', () => { // ❌ Test trop basique
  render(<UserProfile user={null} />);
});
...
function user_profile({ User, on_edit }) { // ❌ snake_case
  const [isediting, setisediting] = useState(false); // ❌ camelCase incorrect
  
  const HandleSave = useCallback((user_data) => { // ❌ PascalCase pour fonction
...
function TodoList() {
  ...
  return (
    <div>
      {/* Logique rendue complexe par la duplication d'état */}
    </div>
  );
}
```
...
function ProductList({ products }) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('filtersChanged', handleFiltersChange);
  }, []);
...
```
***DON'T DO*** : Éviter le rendu impératif
...
```
***DON'T DO*** : Éviter les mutations directes
```jsx
...
```
***DON'T DO*** : Éviter les optimisations prématurées ou incorrectes
```jsx
...
```
***DON'T DO*** : Éviter les mauvaises pratiques avec useEffect
```jsx
...
***DON'T DO*** : Éviter les tests fragiles ou non significatifs
```jsx
