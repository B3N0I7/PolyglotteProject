...
export const validateName = (name: string, fieldName: string): { isValid: boolean; message?: string } => {
    ...
    if (name.trim().length < 2) {
        return { isValid: false, message: `Le ${fieldName.toLowerCase()} doit contenir au moins 2 caractères` };
    }

    if (!/^[a-zA-ZÀ-ÿ\s-]+$/.test(name)) {
        return { isValid: false, message: `Le ${fieldName.toLowerCase()} ne peut contenir que des lettres` };
    }
